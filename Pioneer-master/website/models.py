...
class Expense(models.Model):
    bus_no = models.CharField(max_length=10)
    fuel_required_per_bus = models.IntegerField(validators=[
        MinValueValidator(0)
    ])
    ...
    def total_price(self):
        return self.cost.aggregate(
            total_price=Sum(F('bus_no') * F('fuel_required_per_bus')*F('rate') )
        )['total_price'] or Decimal('0')
   

    def __str__(self):
        return f"Bus No: {self.bus_no} Fuel_req/bus: {self.fuel_required_per_bus} Rate: {self.rate}"
